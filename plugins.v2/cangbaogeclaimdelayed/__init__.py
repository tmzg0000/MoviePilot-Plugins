...
class CangbaoGeClaimDelayed(_PluginBase):
                                ...
                                "content": [
                                    {
                                        ...
                                        "props": {
                                            ...
                                            "hint": "留空时读取站点管理中的藏宝阁PT站点 Cookie；填写后仅本插件使用，不会修改站点 Cookie"
                                        }
                                    }
                                ]
                            },
                                ...
                                "content": [
                                    {
                                        ...
                                        "events": {
                                            "click": {
                                                "api": "plugin/CangbaoGeClaimDelayed/get_cookie",
                                                "method": "get",
                                                "state": "cookie"
                                            }
                                        }
                                ...
                                "content": [
                                    {
                                        ...
                                        "props": {
                                            "type": "info",
                                            "variant": "tonal",
                                            "text": "执行周期支持两种格式：\n1. 标准Cron表达式（如 0 0 1 * * 表示每月1日0点）。\n2. 间隔时间格式：interval:单位:数值（单位支持 d天/h时/m分/s秒，如 interval:d:3 表示每3天执行一次，interval:m:4320 表示每4320分钟执行一次）。\n领取任务后会自动通过系统通知渠道发送消息。"
                                        }
                                    }
                                ]
                            }
    ...
    def run_once_api(self):
        threading.Thread(target=self.claim_task, daemon=True).start()
        return {"status": "started", "message": f"{self.plugin_name}已启动"}
    ...
    def handle_command(self, event: Event):
        ...
        if action == "cangbaoge_claim_delayed":
            threading.Thread(
    ...
    def claim_task(self):
        with self._lock:
            try:
                ...
                if not self._cookie:
                    logger.error(f"{self.plugin_name}：未配置Cookie，无法执行")
                    if self._notify:
                        self.post_message(
                ...
                if result.get("success"):
                    logger.info(f"{self.plugin_name}成功：{result.get('message')}")
                    if self._notify:
                        self.post_message(
                            mtype=NotificationType.SiteMessage,
                            title=f"【{self.plugin_name}】",
                    ...
                    logger.warn(f"{self.plugin_name}失败：{result.get('message')}")
                    if self._notify:
                        self.post_message(
                            mtype=NotificationType.SiteMessage,
                            title=f"【{self.plugin_name}】",
    ...
    def __do_claim(self) -> Dict[str, Any]:
        try:
            ...
            logger.info(f"{self.plugin_name}请求：exam_id={self._exam_id}")
            ...
            logger.info(f"{self.plugin_name}响应: {response.text[:500]}")
    ...
    def __get_site_cookie_detail(self) -> Dict[str, Any]:
        try:
            ...
            site = siteoper.get_by_domain(self.SITE_DOMAIN)
            if not site:
                return {"success": False, "msg": f"未添加藏宝阁PT站点({self.SITE_DOMAIN})！请在站点管理中添加。"}
            cookie = site.cookie
    ...
    def __get_records(self) -> List[dict]:
        try:
            records = self.get_data("claim_delayed_records") or []
        except Exception:
            records = []
        return records[:30]
    ...
    def __save_record(self, record: dict):
        ...
        self.save_data("claim_delayed_records", records[:30])
    ...
    def stop_service(self):
        try:
            Scheduler().remove_plugin_job("CangbaoGeClaimDelayed")
        except Exception:
            pass
